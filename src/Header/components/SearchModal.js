...
function SearchModal(props) {
  ...
  const findOpen = workList.filter((data)=>{
    ...
    return (data.work_title.includes(keyword)) || (data.artist_name.includes(keyword))
  })
    ...
    <>
      ...
      >
        ...
        <Modal.Body style={{height: "100%"}}>
          ...
          <div style={{fontFamily:"NanumSquareB", fontSize: "30px", marginTop: "30px"}}>
            {
              keyword === '' ? <div>검색어를 입력해주세요!</div> : 
              <div>
                <div style={{marginBottom:"30px"}}>모든 프로젝트</div>
                <CardGroup className="cardgroup_Home">
                  {
                    findOpen.slice(0,count).map(function(data, i){ //8개
                      return(
                      <div style={{marginLeft:"20px", marginBottom:"-80px"}}>
                        <Card className="card_Home">
                        <Link to={`/detail/${data.work_no}`} onClick={()=>{mainStore.getWork(data.work_no); setShow(false);}}>
                        ...
                        <Link to={`/detail/${data.work_no}`} onClick={()=>{mainStore.getWork(data.work_no); setShow(false); }}>{data.work_title}</Link>
                        </Card.Text>
                        <div className="cardfooter" style={{fontSize:"18px", textAlign:"center"}}>
                          <p style={{marginTop:"-25px"}}>{Math.round((data.funding_now/data.funding_goal)*100)}% 달성</p>
                        </div>
                        </Card.Body>
                        </Card>
                      </div>
